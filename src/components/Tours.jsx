...
const TourCard = ({ tour, onRemove }) => {
  ...
  const toggleInfo = () => {
    ...
    setButtonLabel(showFullInfo ? "Read More" : "See More");
  };
  ...
  return (
    <div className="card">
      ...
      <div className="card-body">
        ...
        <p className="card-text">{tour.info}
        {showFullInfo ? tour.info : `${tour.info.substring(0, 35)}...`}
        </p>
        <button className="btn btn-secondary" onClick={toggleInfo}>{buttonLabel}</button>
        <p className="card-price">${tour.price}</p>
        <button className="btn btn-primary" onClick={() => onRemove(tour.id)}>Not Interested</button>
